...
def extract_disaster_info(row):
    ...
    kk_match = re.search(r'(\d+)\s*kk', text_lower)
    if kk_match:
        terdampak_kk = kk_match.group(1)

    jiwa_match = re.search(r'(\d+)\s*(jiwa|orang)', text_lower)
    if jiwa_match:
        terdampak_jiwa = jiwa_match.group(1)
    ...
    kk_mengungsi = re.search(r'(\d+)\s*kk.*mengungsi', text_lower)
    if kk_mengungsi:
        mengungsi_kk = kk_mengungsi.group(1)

    jiwa_mengungsi = re.search(r'(\d+)\s*(jiwa|orang).*mengungsi', text_lower)
    if jiwa_mengungsi:
        mengungsi_jiwa = jiwa_mengungsi.group(1)
    ...
    rumah_match = re.search(r'(\d+)\s*rumah', text_lower)
    if rumah_match:
        rumah_rusak = rumah_match.group(1)
    ...
    meninggal_match = re.search(r'(\d+)\s*(orang|jiwa)?\s*(meninggal|tewas)', text_lower)

    if meninggal_match:
        korban_meninggal = meninggal_match.group(1)
    ...
    luka_match = re.search(r'(\d+)\s*(orang|jiwa)?\s*luka', text_lower)

    if luka_match:
        korban_luka = luka_match.group(1)
    ...
    elif "puting beliung" in text_lower or "angin puting beliung" in text_lower:
        jenis = "Puting Beliung"

    elif "gelombang pasang" in text_lower or "rob" in text_lower:
        jenis = "Gelombang Pasang"

    elif "abrasi" in text_lower:
        jenis = "Abrasi"

    elif "longsor" in text_lower:
        jenis = "Tanah Longsor"

    elif "kekeringan" in text_lower:
        jenis = "Kekeringan"

    elif "gempa" in text_lower:
        jenis = "Gempa Bumi"

    elif "gunung meletus" in text_lower or "erupsi" in text_lower:
        jenis = "Gunung Meletus"

    elif "kebakaran hutan" in text_lower or "karhutla" in text_lower:
        jenis = "Kebakaran Hutan"
    kronologis = text if text else "-"

    return pd.Series({
        "Provinsi": provinsi,
        "Jenis Bencana": jenis,
        "Kronologis": kronologis[:300] + "...",
